...
fn main() {
    ...
    println!(
        "part1: {}",
        contents.lines().filter(|line| is_safe(line, false)).count()
    );
    println!(
        "part2: {}",
        contents.lines().filter(|line| is_safe(line, true)).count()
    );
}
...
fn is_safe(line: &str, with_tolerance: bool) -> bool {
    let report: Vec<i64> = line
        ...
        .collect();
    if with_tolerance {
        create_variants(&report)
            .iter()
            .any(|variant| is_safe_report(variant))
    } else {
